...
const dbDir = path.join(__dirname, 'dept_verify_workspace');
...
console.log(`=== STARTING BACKEND FOR DEPARTMENT MASTER VERIFICATION ON PORT ${PORT} ===`);
...
async function runVerification() {
    ...
    console.log('\n--- 1. GET /api/v1/departments WITHOUT TENANT CONTEXT ---');
    const noContextRes = await get('/api/v1/departments');
    console.log('Status (Expected 400 TENANT_CONTEXT_MISSING):', noContextRes.status);
    console.log('Response Body:\n', JSON.stringify(noContextRes.body, null, 2));

    console.log('\n--- 2. POST /api/v1/departments WITHOUT TENANT CONTEXT ---');
    const createNoContextRes = await post('/api/v1/departments', {
    ...
    if (noContextRes.status === 400 && createNoContextRes.status === 400) {
        console.log('DEPARTMENT MASTER TENANT ISOLATION VERIFICATION PASSED SUCCESSFULLY!');
    } else {
